...
# ╔═╡ 569aa133-504f-4bdf-adf2-1156c929edef
md"""# Spring-Mass Harmonic Oscillator"""

# ╔═╡ f65ad4c0-401f-497e-82bc-e1adacfe937f
k = 100

# ╔═╡ 7106f634-90f6-4932-96d7-6d00a78fb936
m = 1

# ╔═╡ 1255a168-ec80-4ad8-ba63-599d0f57aa02
ω = sqrt(k/m)

# ╔═╡ 468d0170-9e18-4068-af0d-01544aa41955
# Simple Harmonic Oscillator Problem
import OrdinaryDiffEq as ODE, Plots

#Initial Conditions

# ╔═╡ 3567a381-1f1a-4f6a-959b-066ee1ac10f4
x0 = 0

# ╔═╡ a93cab3d-a3ee-41b2-84ad-6f1b2a1471a0
v0 = 2

# ╔═╡ 859f9df9-46ad-418b-97bd-7b33fa2d1f72
tspan = (0, 4*pi/ω)

# ╔═╡ 84c11acf-449d-4859-a893-9a5f37cb9abf
A = x0

# ╔═╡ 5ffc7836-48ac-4bb1-a80f-b179f374f681
B = v0/ω

#Define the problem

# ╔═╡ 4cd85e7c-2a6e-4bcd-b99a-9b2981802bac
...
# ╔═╡ 2d6600b3-319f-48a5-84dc-261cd7642f99
...
# ╔═╡ b8be1299-0834-45b2-9e87-818e423340af
sol = ODE.solve(prob, ODE.DPRKN6())

# ╔═╡ 215fd40d-b359-4919-a3f7-c5fc335641e1
t = sol.t  # extracting time from solution

# ╔═╡ 2a6aa1fa-a813-4313-8e25-a42815814311
sin.(t)
#Plot

# ╔═╡ 3cef1e6a-7f2f-47f1-bbf1-ed2fb6e1a54c
...
begin
    ...
    V = 1/2 .* k .* x.^2
    L = T .- V
    Plots.plot(t, T, label = "KE")
    Plots.plot!(t, V, label = "PE")
    Plots.plot!(t, L, label = "Action")
end

# ╔═╡ 87379f73-9494-4745-b5b0-2b580a5ed9bb
function integrate_action(L, t)
    L_dt = L[1:end-1].*(t[2:end] - t[1:end-1])
    return sum(L_dt)
end

# ╔═╡ 0bc65876-8c68-475c-b5f6-d23bcd68e58b
integrate_action(L, t)

# ╔═╡ c949aec0-878e-47f1-a654-3434a947135d
begin
...
# ╔═╡ ff48d765-c46a-45f2-933e-1ac56aae318e
integrate_action(L_an, t)

# ╔═╡ 3e5be2aa-ed3c-453e-a74c-7e2b23b296b6
md""" ## Try My Own Trial Functions """

# ╔═╡ 76076653-376d-430d-a157-92a1a8b659da
# BCs
x_start = x_an[1]

# ╔═╡ 810cc5b0-f680-4628-8439-a55d3ec206d2
x_end   = x_an[end]

# ╔═╡ 40158ccf-5285-43a7-b099-b2cb95e13e30
t_start = t[1]

# ╔═╡ ba04fdd9-c68c-49b5-a456-9c933834701e
t_end   = t[end]

# ╔═╡ 439fcc65-ac92-4fd1-a10f-a328ff866a4f
# Polynomial
function trial_polynomial(t)
    τ = (t .- t_start) ./ (t_end - t_start)
    return x_start .* (1 .- τ) .+ x_end .* τ .+ 0.5 .* τ .* (1 .- τ)
end

# ╔═╡ 3671cff9-79fc-4f08-860f-fa3ff527f594
begin
    x_trial = trial_polynomial(t)

    # numerical derivative for velocity
    v_trial = diff(x_trial) ./ diff(t)
    v_trial = vcat(v_trial, v_trial[end])  # match vector length

    T_trial = 1/2 .* m .* v_trial.^2
    V_trial = 1/2 .* k .* x_trial.^2
    L_trial = T_trial .- V_trial

    action_trial = integrate_action(L_trial, t)
end

# ╔═╡ 4dc05f09-020b-44ce-9c2e-ae22dc308e55
Plots.plot(t, x_an, lw=3, label="True Solution")

# ╔═╡ 5b9260c4-56e4-4759-aa92-608562028fb9
Plots.plot!(t, x_trial, lw=2, ls=:dash, label="Trial Polynomial")

# ╔═╡ d17597cb-52f5-4612-91b0-7a295796a86f
md""" ## Variation: Localized Sine Perturbation """

# ╔═╡ 9e4dbf6e-f0d7-4c02-8de3-ae491aaeabf9
ϵ = 0.15

# ╔═╡ c252b18f-4e0b-4d37-8dab-216b3eb5719b
x_var = x_an .+ ϵ .* sin.(π .* (t .- t_start) ./ (t_end - t_start))

# ╔═╡ 43112675-5f40-4c49-84a8-b1704c3e9440
v_var = diff(x_var) ./ diff(t)

# ╔═╡ 8015632c-3aa8-4a6d-93a9-7efea523d2c3
v_var2 = vcat(v_var, v_var[end])

# ╔═╡ d014558c-70b1-43a4-9ba5-131fedfb0265
T_var = 1/2 .* m .* v_var2.^2

# ╔═╡ 2ebfdb98-53b2-4c50-a3ff-089257cc1e5d
V_var = 1/2 .* k .* x_var.^2

# ╔═╡ 066fac0d-c846-488d-9b31-2043f0bb2c1c
L_var = T_var .- V_var

# ╔═╡ f3bc1265-0141-40ed-94f7-af38840ceea4
action_var = integrate_action(L_var, t)

# ╔═╡ 0ab959b0-fd41-467f-8625-88464323a791
println("Action (Sine Variation)     = ", action_var)

# ╔═╡ eca72395-1da5-4bd4-9ba7-6325ffe4f9c3
Plots.plot(t, x_an, lw=3, label="True Solution")

# ╔═╡ dc1f00c0-6e0a-4f3f-882b-4e18587c6d75
Plots.plot!(t, x_var, lw=2, ls=:dash, label="Sine Perturbation")

# ╔═╡ 7c620900-c603-4ab8-95a7-7622c9424e50
md""" ## Extreme Variation: High-Frequency Oscillation """

# ╔═╡ 0c4cbd85-2603-4e78-a7dd-e316203d0694
ϵ_extreme = 0.3

# ╔═╡ c45200c0-1298-40ef-ab85-04f6007320c7
N = 20 

# ╔═╡ 3176c762-9c67-4fe0-a699-6e45cd483584
x_extreme = x_an .+ ϵ_extreme .* sin.(N * π .* (t .- t_start) ./ (t_end - t_start))

# ╔═╡ 3d86a824-9feb-408c-abf5-74516369e8ea
v_extreme = diff(x_extreme) ./ diff(t)

# ╔═╡ b1de22d2-a562-40c2-8371-950d993c84ac
v_extreme2 = vcat(v_extreme, v_extreme[end])

# ╔═╡ be6f6cc6-a63b-4f31-972b-06b1c9b24040
T_extreme = 1/2 .* m .* v_extreme2.^2

# ╔═╡ 57d4127d-2177-4f54-a61c-1b1d1827229b
V_extreme = 1/2 .* k .* x_extreme.^2

# ╔═╡ 28ba94d5-4ed3-485a-858a-a5d635c71744
L_extreme = T_extreme .- V_extreme

# ╔═╡ 13f2fcb3-c4b2-405c-acc0-90e7233db230
action_extreme = integrate_action(L_extreme, t)

# ╔═╡ 12020d6b-c53c-42d7-a899-63f9376f24e5
println("Action (True Solution)      = ", action_true)

# ╔═╡ 5af207fb-45bc-475c-a5a8-a6335cae08ff
println("Action (True Solution)      = ", action_true)

# ╔═╡ 9dc8481c-b44d-4439-8c67-5376c09280c3
println("Action (Extreme Variation)  = ", action_extreme)

# ╔═╡ da3a8060-4cd9-4228-8556-c1095adf8028
Plots.plot(t, x_an, lw=3, label="True Solution")

# ╔═╡ 295487d1-e813-4783-a589-48ce3f64a8b5
Plots.plot!(t, x_extreme, lw=2, ls=:dash, label="Extreme High-Frequency Path")

# ╔═╡ 8ab9fe84-4320-4ea5-8db5-a6d058bd9177


# ╔═╡ 272d3214-dc24-4849-8b60-a31388ed38c0
action_true = integrate_action(L_an, t)

# ╔═╡ 365fab62-8dd1-435e-a75c-57460cfcbeb6
# ╠═╡ disabled = true
#=╠═╡
begin
    action_true = integrate_action(L_an, t)
    ...
    println("Action (True Solution)  = ", action_true)
    println("Action (Trial Function) = ", action_trial)
end
  ╠═╡ =#

# ╔═╡ b97a4127-692b-48ca-8bbf-6f4c25cea190
# ╠═╡ disabled = true
#=╠═╡
action_true = integrate_action(L_an, t)
  ╠═╡ =#
...
manifest_format = "2.0"
project_hash = "ab23d90b169a1e47c7c7e51bd90fc303eac92ad7"
