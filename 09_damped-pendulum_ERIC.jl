...
# ╔═╡ e5884190-4631-11f1-9a55-b583ff3d0a67
begin
	using Symbolics
...
# ╔═╡ 59c1e840-5ddb-4b8c-b9ac-f7b46e139878
md"""
# Lagrange Equation Solver Project

The goal is to build a general workflow that starts from the Lagrangian:

``\mathcal{L} = T - V``

Then uses the Euler-Lagrange equation with non-conservative generalized forces
"""

# ╔═╡ 97bcb1a6-97cb-4c79-b440-743f57b8c60c
md"""
...
- ``b`` = support damping coefficient

The pendulum is modeled as a slender rod, so its rotational inertia about its center of mass is already included in the kinetic energy expression.

"""

# ╔═╡ bdfdb827-c890-499a-bb89-b670e2f258b3
begin
	# Physical parameters
	m = 0.4      # kg
	L = 1.0      # m
...
# ╔═╡ 7fd5157c-10e3-4525-a7b9-e682f20326e0
md"""
## Step 2 — Define the Lagrangian

The position of the pendulum center of mass is written using the generalized coordinates ``x`` and ``\theta``.

For this model:

```math
r_x = x + \frac{L}{2}\sin\theta
"""

# ╔═╡ 17347bcf-483a-4484-82c4-4e1bdaa28b0a
begin
	@variables t
	@variables x(t) θ(t)
	D = Differential(t)
	...
	vx = xdot + (L/2)*θdot*cos(θ)
	vy = (L/2)*θdot*sin(θ)
	
	T = (1/2)*m*(vx^2 + vy^2) + (1/2)*I_G*θdot^2
	V = (1/2)*k*x^2 + m*g*(L/2)*(1 - cos(θ))
	
	Lag = T - V
end

# ╔═╡ 0aed7ff7-7d32-4677-95a4-75e769a79353
md"""
## Step 3 — General Euler-Lagrange Function

This function takes:

1. a Lagrangian ``\mathcal{L}``
2. a list of generalized coordinates
3. a list of non-conservative generalized forces

and returns the Euler-Lagrange equations.
"""

# ╔═╡ cfc26a23-e880-42af-a0db-428173f6811f
begin
	function lagrange_equations(Lag, qs, Fqs, D)
		eqs = []
		...
		for i in eachindex(qs)
			...
			dL_dqdot = Symbolics.derivative(Lag, qdot)
			dL_dq = Symbolics.derivative(Lag, q)
			
			eq = expand_derivatives(D(dL_dqdot)) - dL_dq - Fq
			
			push!(eqs, simplify(eq))
		end
...
# ╔═╡ fa166c42-9e7e-4139-92a5-18b410b8e92f
md"""
## Step 4 — Add Non-Conservative Generalized Forces

The spring force is conservative, so it is already included in ``V``.

The damper is non-conservative and acts in the ``x`` direction:

```math
F_x = -b\dot{x}
"""

# ╔═╡ b136def5-8171-4780-aaa7-cde1f031c138
begin
	qs = [x, θ]
	Fqs = [-b*D(x), 0.0]
	
	eqs_symbolic = lagrange_equations(Lag, qs, Fqs, D)
end

# ╔═╡ 07b517f5-a0ec-49d8-9ac8-cd95e8da8b3b
md"""
## Symbolic Differential Equations

The two equations below are the symbolic equations generated from the Lagrangian.
"""

# ╔═╡ 474c3561-4cfd-473d-b324-18b961f119c7
eqs_symbolic

# ╔═╡ 4cc0c7a0-5a62-48a7-a280-6695dd46d87e
md"""
## Step 5 — Convert the Equations to a Numerical ODE Solver

For numerical solving, define the state vector:
...
# ╔═╡ ee644b52-2ffb-4336-8783-3b7cecc0c813
begin
	function rhs_lagrange!(du, u, p, t)
		x, θ, xdot, θdot = u
...
# ╔═╡ 80cc3202-5d89-4078-bb5a-02232f6beff8
md"""
...
# ╔═╡ adec8913-2982-4c4d-8474-ac11b9d52f81
begin
	u0 = [
		0.15;
		deg2rad(45.0);
	...
	prob = ODEProblem(rhs_lagrange!, u0, tspan, p)
...
# ╔═╡ 8466064c-d404-4dd0-97ce-0310b7677403
md"""
# Results and Visualization
"""

# ╔═╡ 9447c50a-8839-448c-964b-e43c85ad8018
begin
	t_vec = sol.t
	
	x_vec = [u[1] for u in sol.u]
	θ_vec = [u[2] for u in sol.u]
	xdot_vec = [u[3] for u in sol.u]
	θdot_vec = [u[4] for u in sol.u]
end

# ╔═╡ 8aa09bb8-2f5b-4191-a432-05b257fc3e7f
md"""
## Plot 1 — Sliding Support Motion

This plot shows the horizontal spring-mass motion of the support.
"""

# ╔═╡ 19eb5207-b22f-47e4-95bf-92409ef955c8
begin
	plot(
		t_vec,
		x_vec,
		...
		title = "Sliding Support Motion",
		label = "x(t)",
		grid = true
	)
end

# ╔═╡ fcc96240-df6c-4851-b453-a8326f286f3c
md"""
## Plot 2 — Pendulum Angle

This plot shows how the pendulum angle changes over time.
"""

# ╔═╡ 7c2b84f7-2754-4320-8915-4aff98ab004c
begin
	plot(
		t_vec,
		rad2deg.(θ_vec),
		...
		title = "Compound Pendulum Angle",
		label = "θ(t)",
		grid = true
	)
end

# ╔═╡ 885893ec-d5d1-490b-a10c-617c31fe40ae
md"""
## Plot 3 — Phase Plot

This figure shows the pendulum phase space:

```math
\theta \ \text{vs.} \ \dot{\theta}
"""

# ╔═╡ 9dee81f4-1613-4c2a-838c-a35a36241919
begin
	plot(
		rad2deg.(θ_vec),
		rad2deg.(θdot_vec),
...
# ╔═╡ 22776c2a-740f-44a5-a11f-653a3ae832fd
md"""
## Plot 4 — Energy Decay
...
# ╔═╡ 10d3a5ec-a78b-45fa-8e49-d7891247e68d
begin
	function energy(u, p)
		x, θ, xdot, θdot = u
	...
	E_vec = [energy(u, p) for u in sol.u]
end

# ╔═╡ 3029d4e2-38a5-4b68-b088-6f91abbb87b2
begin
	plot(
		t_vec,
		E_vec,
		...
		title = "Energy Decay Due to Damping",
		label = "E = T + V",
		grid = true
	)
end

# ╔═╡ 529ce25c-5535-4223-a24b-5dd30aa1d4f4
md"""
## Plot 5 — Motion Path of the Pendulum Center of Mass

The center of mass of the pendulum is:

```math
r_x = x + \frac{L}{2}\sin\theta
"""

# ╔═╡ 84f51376-8d07-457b-9499-e577aaac1727
begin
	rx_vec = x_vec .+ (L/2).*sin.(θ_vec)
	ry_vec = .-(L/2).*cos.(θ_vec)
	...
	plot(
		...
		xlabel = "x-position (m)",
		ylabel = "y-position (m)",
		title = "Pendulum Center of Mass Path",
		label = "COM path",
		aspect_ratio = :equal,
		grid = true
	)
...
# ╔═╡ 35ad97ef-55bf-47f2-b1ae-39803a86d172
md"""
## System Configuration Snapshots

This figure shows several physical positions of the system during the simulation.

The black line represents the pendulum bar.  
The marker represents the center of mass of the pendulum.
"""

# ╔═╡ 61a345d7-ec2b-4a28-b006-06ea3fda82ff
begin
	snapshot_times = [0.0, 2.0, 4.0, 6.0, 8.0, 12.0]
	snapshot_indices = [argmin(abs.(t_vec .- ts)) for ts in snapshot_times]
	
	plt = plot(
		xlabel = "x-position (m)",
		ylabel = "y-position (m)",
		title = "Pendulum Configuration Snapshots",
		aspect_ratio = :equal,
		grid = true,
		legend = :outerright
	)
	...
	for idx in snapshot_indices
		...
		plot!(
			...
			[yA, yB],
			linewidth = 2,
			label = "t = $(round(t_vec[idx], digits=1)) s"
		)
...
# ╔═╡ da55ae74-50bf-4e93-bf0a-68b8ebde2977


# ╔═╡ 5cc5cef4-e662-4ad1-b788-4cb7d5459002


# ╔═╡ 87426036-f388-4f50-9a53-3d1959ad1dec


# ╔═╡ b89a6604-225f-4aa3-bdd9-67403295e0f8
...
manifest_format = "2.0"
project_hash = "5049b8520d764e493e6947e05cff41721d3adcd3"
