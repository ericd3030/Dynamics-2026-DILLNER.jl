...
# ╔═╡ 53047ebe-4644-11f1-acd1-53a96c591af0
begin
...
# ╔═╡ aa95698e-71a1-4315-920d-9f3bece4ae83
md"""
# Damped Pendulum Design Study

This notebook answers the two discussion questions:

1. What combination of stiffness ``k`` and damping ``b`` stops the pendulum from moving best?
2. What other parameters should be considered in the analysis?

The system is a compound pendulum attached to a sliding base. The base has a spring and damper. The motion is coupled because the pendulum moves the base, and the base motion affects the pendulum.

The state vector is

``u = [x, \theta, \dot{x}, \dot{\theta}]``

where:

- ``x`` is the horizontal base displacement
- ``\theta`` is the pendulum angle
- ``\dot{x}`` is the base velocity
- ``\dot{\theta}`` is the pendulum angular velocity
"""

# ╔═╡ 6a769a73-0dda-487b-b2e0-71eb161c8974
begin
	# Physical constants
	system_params = (
		m = 0.1,      # kg, pendulum mass
		L = 0.9,      # m, pendulum length
	...
	initial_state = [x0, θ0, xdot0, θdot0]
	time_span = (0.0, 10.0)
end

# ╔═╡ 2ddb619c-5ba5-4a9e-a4c7-ca5414e5da30
md"""
# Equations of Motion

The coupled equations can be written in mass-matrix form:
...
``\ddot{q} = [\ddot{x}, \ddot{\theta}]``.

The mass matrix is

``M(\theta) =
\begin{bmatrix}
m & \frac{mL}{2}\cos\theta \\
\frac{mL}{2}\cos\theta & \frac{mL^2}{3}
\end{bmatrix}``
...
``f(q,\dot{q}) =
\begin{bmatrix}
\frac{mL}{2}\dot{\theta}^{2}\sin\theta - b\dot{x} - kx \\
-\frac{mgL}{2}\sin\theta
\end{bmatrix}``

At each time step, the solver finds

``\ddot{q} = M^{-1}f``.
"""

# ╔═╡ fbf251c0-a7ac-41d1-8d39-81a11643c6d5
begin
...
# ╔═╡ fdec2126-3204-4398-9f5f-c52f5c2133bd
begin
	function run_case(k_value, b_value)
		params = (
			b = b_value,
			k = k_value,
			m = system_params.m,
...
# ╔═╡ 5cdc127a-43c6-4f69-b612-2d3051fe0ad7
md"""
# Defining the Best Design

To answer the question, I need to define what “best” means.

I define the best ``k`` and ``b`` combination as the case where the pendulum angle and angular velocity are smallest near the end of the simulation.

The score uses:

- RMS pendulum angle near the end
- RMS angular velocity near the end
- RMS base displacement near the end
- final angle
- final angular velocity

A lower score means the pendulum stopped better.
"""

# ╔═╡ dd79ce0c-3f52-40b1-be50-8feb98cc442b
begin
	function performance_score(solution)
		t = solution.t
		...
		t_cut = 0.70 * maximum(t)
		tail_index = findall(t .>= t_cut)
		
		θ_tail = θ[tail_index]
		dθ_tail = dθ[tail_index]
		x_tail = x[tail_index]
		
		rms_θ = sqrt(mean(θ_tail .^ 2))
		rms_dθ = sqrt(mean(dθ_tail .^ 2))
		rms_x = sqrt(mean(x_tail .^ 2))
		
		final_θ = abs(θ[end])
		final_dθ = abs(dθ[end])
		
		score = 10*rms_θ + rms_dθ + 2*rms_x + 10*final_θ + final_dθ
		
		return score
	end
end

# ╔═╡ 9e90014c-992f-4468-84c3-03650fc5e530
begin
	k_values = range(0.2, 20.0, length = 45)
	b_values = range(0.05, 10.0, length = 45)
	
	score_matrix = zeros(length(k_values), length(b_values))
	
	for i in eachindex(k_values)
		for j in eachindex(b_values)
			k_test = k_values[i]
			b_test = b_values[j]
			
			sol_test = run_case(k_test, b_test)
			score_matrix[i, j] = performance_score(sol_test)
		end
	end
	
	best_score, best_index = findmin(score_matrix)
	
	best_k = k_values[best_index[1]]
	best_b = b_values[best_index[2]]
	
	best_solution = run_case(best_k, best_b)
	
	(best_k, best_b, best_score)
end

# ╔═╡ be9b0040-2935-4429-9cb3-87b24a13e14b
md"""
# Idea 1 — Design Map

This heatmap shows the score for each stiffness and damping combination.

The darker/lower-score region represents better stopping behavior. The star marks the best combination found in the sweep.

This plot is useful because it shows that there is not just one random best point. Instead, there is usually a region where the system performs better.
"""

# ╔═╡ 8c59128f-b103-4609-ab02-69c79612ea83
begin
	P_design = heatmap(
		b_values,
		k_values,
		score_matrix,
		xlabel = "Damping coefficient b",
		ylabel = "Spring stiffness k",
		title = "Design Map: Lower Score Means Better Stopping",
		colorbar_title = "motion score",
		label = false
	)
...
# ╔═╡ 0e0f1fbd-1427-4b88-8933-3174d4b19b87
begin
	t_best = best_solution.t
	
	x_best = best_solution[1, :]
	θ_best = best_solution[2, :]
	dx_best = best_solution[3, :]
	dθ_best = best_solution[4, :]
end

# ╔═╡ 46ef8b16-885a-443b-9e0d-96ee17f1146d
begin
	plot(
		t_best,
		rad2deg.(θ_best),
		linewidth = 3,
		xlabel = "Time (s)",
		ylabel = "Pendulum angle θ (deg)",
		title = "Best Design Pendulum Angle Response",
		label = "k = $(round(best_k, digits=2)), b = $(round(best_b, digits=2))",
		grid = true
	)
end

# ╔═╡ daf6dd10-3238-4c82-a1c7-eecae3c47493
begin
	plot(
		t_best,
		x_best,
		linewidth = 3,
		xlabel = "Time (s)",
		ylabel = "Support displacement x (m)",
		title = "Best Design Support Motion",
		label = "x(t)",
		grid = true
	)
end

# ╔═╡ c45e8022-dde2-4416-b4fb-6df72289b455
md"""
# Idea 2 — Why Maximum Damping Is Not Always Best

This plot compares three damping cases:

1. low damping
2. best damping from the sweep
3. very high damping

The high damping case can perform worse because it nearly locks the base in place. When the base cannot move, less pendulum energy is transferred into the damper.

Therefore, the optimum damping is not automatically the largest damping coefficient.
"""

# ╔═╡ a9359e65-87ad-48d0-96be-bbb60132cea8
begin
	k_compare = best_k
	...
	sol_low = run_case(k_compare, b_low)
	sol_best = run_case(k_compare, best_b)
	sol_high = run_case(k_compare, b_high)
	
	P_compare = plot(
		xlabel = "Time (s)",
		ylabel = "Pendulum angle θ (deg)",
		title = "Low vs Best vs Very High Damping",
		grid = true
	)
	...
	plot!(
		...
		rad2deg.(sol_best[2, :]),
		linewidth = 3,
		label = "best damping b = $(round(best_b, digits=2))"
	)
	...
	plot!(
		...
		rad2deg.(sol_high[2, :]),
		linewidth = 2,
		label = "very high damping b = $(b_high)"
	)
	
	P_compare
end

# ╔═╡ 7ff644c4-5985-4c7a-a1b9-d86abda7fc56
md"""
# Energy Removed by the Damper
...
Because the system includes damping, the total mechanical energy should decrease over time.
"""

# ╔═╡ 714e0b54-62b2-4caf-9276-1d2986ef59cd
begin
	function total_energy(y, k_value)
		x, θ, dx, dθ = y
		...
		T = 1/2*m*(vx^2 + vy^2) + 1/2*I_G*dθ^2
		V = 1/2*k_value*x^2 + m*g*L/2*(1 - cos(θ))
		
		return T + V
	end
	
	E_best = [total_energy(u, best_k) for u in best_solution.u]
end

# ╔═╡ c3bf4a1e-f718-4a8a-a752-50bade1911d1
begin
	plot(
		t_best,
		E_best,
		...
		title = "Energy Removed by the Damper",
		label = "E = T + V",
		grid = true
	)
end
...
manifest_format = "2.0"
project_hash = "ef9f2cb058c15ded8ed3d949ce520fca3d98c02b"
