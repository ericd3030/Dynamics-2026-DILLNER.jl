...
# ╔═╡ 7571b058-bd26-4164-a70a-66a610a4d4ce
begin
...
# ╔═╡ f0455371-be86-48fb-9df3-f3255716ab13
md"""
# Door Kinematic Constraints

This notebook studies different ways to constrain a door opening motion.

The door is treated as a rigid body moving in the plane.
...
- ``x`` is the horizontal position of the door center
- ``y`` is the vertical position of the door center
- ``\theta`` is the door rotation angle

The goal is to solve constraint equations of the form

``C(q,t)=0``

For a 2D rigid body, there are three generalized coordinates. Therefore, if we add three independent constraints, the motion is fully prescribed. This creates a zero-degree-of-freedom kinematic problem.
"""

# ╔═╡ b7bf1a61-1ab1-4562-ac82-9ea628851beb
begin
	# Door geometry
	W = 1.0          # door width
	T = 0.05         # door thickness
	
	# Fixed hinge location in ground coordinates
	hinge_x = 0.0
	hinge_y = 0.0
	
	# Simulation time
	time = LinRange(0.0, 1.0, 101)
end

# ╔═╡ 851e3c68-b954-4d26-8124-36f4470a05f2
md"""
# Door Geometry

A point on the door can be described using a body-fixed location ``r_local``.

To convert that point into the ground frame:

``r_global = [x,y] + A(\theta)r_{local}``
...
- the hinge is on the left edge of the door
- the knob is on the right edge of the door
- the door center is the body origin
"""

# ╔═╡ 20079362-ba80-431b-b7d3-0fccc547edc4
begin
	function Arot(θ)
		return [
			cos(θ)  -sin(θ);
	...
	function point_on_door(q, r_local)
		x, y, θ = q
		return [x, y] + Arot(θ)*r_local
	end
	
	# Local body-fixed points
	hinge_local = [-W/2, 0.0]
	knob_local  = [ W/2, 0.0]
	
	function hinge_position(q)
		return point_on_door(q, hinge_local)
	end
	
	function knob_position(q)
		return point_on_door(q, knob_local)
	end
end

# ╔═╡ eecb70c8-e0e0-4798-9024-065ff7568cbb
md"""
# Base Hinge Constraints

The hinge point on the door must stay attached to the fixed hinge point in the ground frame.

The hinge point on the door is

``r_h(q) = [x,y] + A(\theta)[-W/2,0]``

The fixed hinge point is
...
These two equations force the door to rotate about the hinge instead of translating freely.
"""

# ╔═╡ 1e49933f-f1fe-4a58-b809-60b14b67edd9
md"""
# Three Different Door Constraints

After the two hinge constraints, we still need one more constraint to prescribe the motion.

I tested three different third constraints:

## Case A — Prescribed angle

The door angle is directly prescribed:

``\theta = \theta_{des}(t)``

This is the simplest way to open the door.

## Case B — Prescribed knob x-position

The knob is forced to move horizontally according to a desired function:

``x_{knob} = x_{des}(t)``

This answers the question directly using the door knob.

## Case C — Prescribed knob y-position

The knob is forced to move vertically according to a desired function:

``y_{knob} = y_{des}(t)``

This creates a different opening motion and shows how changing the constraint changes the result.
"""

# ╔═╡ 975fc851-d199-4c4d-a8f2-caafa5adec9f
begin
	# Smooth ramp from 0 to 1
	# This avoids a sudden jump at the start.
	function s_ramp(t)
		return 3t^2 - 2t^3
	end
	
	# Desired angle for Case A
	function θ_des(t)
		return deg2rad(80.0)*s_ramp(t)
	end
	
	# Desired knob x-position for Case B
	# The knob starts at x = W and moves inward as the door opens.
	function knob_x_des(t)
		return W*cos(θ_des(t))
	end
	
	# Desired knob y-position for Case C
	# The knob starts at y = 0 and moves upward as the door opens.
	function knob_y_des(t)
		return W*sin(θ_des(t))
	end
end

# ╔═╡ da92431c-ecda-48ef-83d4-cb9334e2e58f
begin
	function C_angle(q, t)
		h = hinge_position(q)
...
# ╔═╡ 1a28896c-21e6-4eca-88f3-e7825f7ee822
md"""
# Solving the Position Constraints

At each time step, the code solves

``C(q,t)=0``

The previous time step is used as the initial guess for the next time step. This makes the nonlinear solve more stable because the door motion changes smoothly.
"""

# ╔═╡ 0cf20a5e-4894-46b1-bcfc-160172fe8578
begin
	function solve_constraint_path(Cfun; q_start = [W/2, 0.0, 0.0])
		q_all = zeros(length(time), 3)
		residual_all = zeros(length(time))
		...
		for i in eachindex(time)
			...
			problem = NonlinearProblem((q, p) -> Cfun(q, p), guess, ti)
			solution = solve(problem)
			
			q_sol = solution.u
			
			q_all[i, :] .= q_sol
			residual_all[i] = norm(Cfun(q_sol, ti))
			
			guess = q_sol
		end
		
		return q_all, residual_all
	end
end

# ╔═╡ 791c3cd8-dead-40ad-bd73-9e3b967cbfe7
begin
	q_angle, residual_angle = solve_constraint_path(C_angle)
	q_knob_x, residual_knob_x = solve_constraint_path(C_knob_x)
	q_knob_y, residual_knob_y = solve_constraint_path(C_knob_y)
	
	"Done solving all three constraint cases"
end

# ╔═╡ 3423cfc0-3208-4636-95b4-628e3cfca482
begin
	x_angle = q_angle[:, 1]
	y_angle = q_angle[:, 2]
	θ_angle = q_angle[:, 3]
...
# ╔═╡ d526b27d-4847-45d3-8a6c-2f4bedfd1a77
begin
	plot(
		time,
		rad2deg.(θ_angle),
		linewidth = 3,
		label = "Case A: prescribed angle",
		xlabel = "Time",
		ylabel = "Door angle θ (deg)",
		title = "Door Angle for Different Constraint Choices",
		grid = true
	)
	...
	plot!(
		...
		rad2deg.(θ_knob_x),
		linewidth = 2,
		label = "Case B: prescribed knob x"
	)
	...
	plot!(
		...
		rad2deg.(θ_knob_y),
		linewidth = 2,
		label = "Case C: prescribed knob y"
	)
end

# ╔═╡ c005dd72-20db-4573-8e2d-9c87de4aec98
begin
	plot(
		x_angle,
		y_angle,
		linewidth = 3,
		label = "Case A: prescribed angle",
		xlabel = "Door center x",
		ylabel = "Door center y",
		title = "Door Center Path",
		aspect_ratio = :equal,
		grid = true
	)
...
# ╔═╡ b05f2d44-151e-4453-b07b-834756204dc1
begin
	function knob_path(q_all)
		kx = zeros(size(q_all, 1))
		ky = zeros(size(q_all, 1))
		
		for i in 1:size(q_all, 1)
			kp = knob_position(q_all[i, :])
			kx[i] = kp[1]
			ky[i] = kp[2]
		end
	...
	kx_angle, ky_angle = knob_path(q_angle)
	kx_knob_x, ky_knob_x = knob_path(q_knob_x)
	kx_knob_y, ky_knob_y = knob_path(q_knob_y)
end

# ╔═╡ ec31a8cd-43a2-491a-a5c0-50e185b9689e
begin
	plot(
		kx_angle,
		ky_angle,
		linewidth = 3,
		label = "Case A: prescribed angle",
		xlabel = "Knob x-position",
		ylabel = "Knob y-position",
		title = "Door Knob Path",
		aspect_ratio = :equal,
		grid = true
	)
...
# ╔═╡ d37d08a0-4654-11f1-9900-4921defc857d
begin
	plot(
		kx_angle,
		ky_angle,
...
# ╔═╡ c057b1c5-0284-4d2d-828c-3f09d48c3afc
begin
	plot(
		time,
		residual_angle,
...
# ╔═╡ b074d5af-3cf5-4f01-8a98-45febd31a2e6
begin
	function door_corners(q)
		x, y, θ = q
		...
		global_corners = zeros(size(local_corners))
		
		for i in 1:size(local_corners, 1)
			global_corners[i, :] .= [x, y] + Arot(θ)*local_corners[i, :]
		end
		
		return global_corners
	end
	
	function plot_door_snapshots(q_all, title_text)
		snapshot_times = [0.0, 0.25, 0.5, 0.75, 1.0]
		snapshot_indices = [argmin(abs.(time .- ts)) for ts in snapshot_times]
		...
		for idx in snapshot_indices
			c = door_corners(q_all[idx, :])
			kp = knob_position(q_all[idx, :])
			...
			plot!(
				...
				c[:, 2],
				linewidth = 2,
				label = "t = $(round(time[idx], digits=2))"
			)
...
# ╔═╡ 5486aba5-ec88-4c46-92da-3fb6e710e43e
begin
	plot_door_snapshots(q_angle, "Door Snapshots: Prescribed Angle Constraint")
end

# ╔═╡ 52d426c4-7ba7-4983-af30-6089cda4404f
begin
	plot_door_snapshots(q_knob_x, "Door Snapshots: Prescribed Door Knob x-Constraint")
end

# ╔═╡ 05adaa54-99b7-47f3-b08a-4af81aefd857
begin
	plot_door_snapshots(q_knob_y, "Door Snapshots: Prescribed Door Knob y-Constraint")
end
