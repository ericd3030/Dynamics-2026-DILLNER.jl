...
# ╔═╡ 55fcae20-465a-11f1-a504-cb0afff59ea9
begin
...
# ╔═╡ 33820178-ee70-416c-96a5-efa467a31e21
md"""
# Piston-Crank Kinematic Model

This notebook recreates the piston-crank kinematic constraint setup.

The system has four bodies:

1. ground support
2. crankshaft / crank arm
3. connecting rod
4. piston slider

Each body has three planar coordinates:

``x, y, \theta``

So the full generalized coordinate vector is

``q = [x_1,y_1,\theta_1,x_2,y_2,\theta_2,x_3,y_3,\theta_3,x_4,y_4,\theta_4]``

The goal is to solve:

``C(q,t)=0``

then solve for the velocities using:

``C_q\dot{q}=-C_t``

Finally, we estimate accelerations and recreate the angular acceleration plots for the crankshaft and connecting rod.
"""

# ╔═╡ 70bcfafd-4e3b-480d-a893-d0c6b2e4971c
begin
	# Geometry from the tutorial-style piston crank example
	...
	# Time vector similar to the book figure range
	t_start = 0.0
	t_end = Trev
	N = 301
	tN = LinRange(t_start, t_end, N)
	
	(L2, L3, ω0, Trev)
end

# ╔═╡ db93ee3f-96d8-4ef8-92e9-5e106afbf16e
md"""
# Prescribed Crankshaft Rotation

If the crankshaft angle is prescribed as a perfect constant-speed rotation,

``\theta_2 = \omega_0 t``

then the crankshaft angular acceleration is zero.

However, the textbook plot you attached shows a crankshaft angular acceleration that changes with time. To reproduce that style of graph, I prescribe a slightly non-uniform crank rotation:

``\theta_2(t)=\omega_0t+a_1\sin(2\pi t/T)+a_2\sin(4\pi t/T)``

This keeps the motion periodic but gives the crankshaft a nonzero angular acceleration.

The connecting rod acceleration is then solved from the piston-crank constraints.
"""

# ╔═╡ b2a3c8c5-36f3-416d-8667-f97cc6c1493b
begin
	# These two terms create a non-uniform prescribed crank rotation.
	# Increase these values if you want larger angular acceleration peaks.
	a1 = 0.70
	a2 = -0.25
	
	function θ2_drive(t)
		return ω0*t + a1*sin(2*pi*t/Trev) + a2*sin(4*pi*t/Trev)
	end
	
	function θ2dot_drive(t)
		return ω0 + a1*(2*pi/Trev)*cos(2*pi*t/Trev) + a2*(4*pi/Trev)*cos(4*pi*t/Trev)
	end
	
	function θ2ddot_drive(t)
		return -a1*(2*pi/Trev)^2*sin(2*pi*t/Trev) - a2*(4*pi/Trev)^2*sin(4*pi*t/Trev)
	end
end

# ╔═╡ 8518d78e-affc-40c1-9213-d2e2424da586
md"""
# Constraint Equations

The constraints are built from the piston-crank geometry.

The ground body is fixed:
...
The crank is pinned to the ground.

The connecting rod is pinned to the crank tip.

The piston is pinned to the right end of the connecting rod and is constrained to slide horizontally.

The final constraint prescribes the crankshaft angle:

``\theta_2-\theta_{2,drive}(t)=0``
"""

# ╔═╡ f31778d1-7cc3-4066-88cf-6fd0d86c0609
begin
	function C(q, t)
		x1, y1, θ1, x2, y2, θ2, x3, y3, θ3, x4, y4, θ4 = q
		...
		return [
			...
			x3 - L3/2*cos(θ3) - x2 - L2/2*cos(θ2);
			y3 - L3/2*sin(θ3) - y2 - L2/2*sin(θ2);
...
# ╔═╡ 19e846bd-b41a-44d6-957c-7652329d2137
begin
	function initial_guess_from_angle(θ2)
		# Crank center of mass
		x1 = 0.0
		y1 = 0.0
		θ1 = 0.0
		...
		# Slider lies on y = 0
		x4 = x_tip + sqrt(L3^2 - y_tip^2)
		y4 = 0.0
		θ4 = 0.0
		
		# Connecting rod angle
		θ3 = atan(-y_tip, x4 - x_tip)
	...
	q0_guess = initial_guess_from_angle(θ2_drive(0.0))
end

# ╔═╡ 4b8553f1-0a40-45a7-b077-627be4ffdd5d
md"""
# Position Solve

At every time step, the nonlinear system

``C(q,t)=0``

is solved.

The previous solution is used as the initial guess for the next time step. This makes the solution path smooth and repeatable.
"""

# ╔═╡ 81c9efb5-6896-47ce-b8b6-bb623e14041e
begin
	function solve_positions()
		q_sol = zeros(length(tN), 12)
		residual = zeros(length(tN))
		...
		for i in eachindex(tN)
			...
			problem = NonlinearProblem((u, p) -> C(u, p), guess, ti)
			solution = solve(problem)
			
			q_current = solution.u
			
			q_sol[i, :] .= q_current
			residual[i] = norm(C(q_current, ti))
			
			guess = q_current
		end
		
		return q_sol, residual
	end
	
	q_sol, residual = solve_positions()
end

# ╔═╡ faa31ca4-5e50-4166-bbb7-2ee080ed7737
md"""
# Velocity Solve

The velocity equation comes from differentiating the constraints:
...
The code below computes ``C_q`` and ``C_t`` numerically and solves the linear system at each time step.
"""

# ╔═╡ 9df5a639-a491-47d9-bb2a-751dad08e243
begin
	function jacobian_q(Cfun, q, t; h = 1e-7)
		m = length(Cfun(q, t))
		n = length(q)
		J = zeros(m, n)
		...
		for j in 1:n
			...
			J[:, j] .= (Cfun(q .+ dq, t) .- Cfun(q .- dq, t)) ./ (2h)
		end
		
		return J
	end
	
	function partial_t(Cfun, q, t; h = 1e-7)
		return (Cfun(q, t + h) .- Cfun(q, t - h)) ./ (2h)
	end
end

# ╔═╡ 34134146-c56a-4894-8254-c165f43b2618
begin
	function solve_velocities(q_sol)
		dq_sol = zeros(size(q_sol))
		
		for i in eachindex(tN)
			ti = tN[i]
			qi = q_sol[i, :]
			
			Cq = jacobian_q(C, qi, ti)
			Ct = partial_t(C, qi, ti)
	...
	dq_sol = solve_velocities(q_sol)
end

# ╔═╡ 68800a1a-aaf7-49f6-b516-192d0d19050d
md"""
# Acceleration Calculation

The textbook plots are angular acceleration plots.

After solving the velocities, I estimate acceleration by differentiating the velocity results with respect to time.

This gives:

``\ddot{q} \approx \frac{d\dot{q}}{dt}``
"""

# ╔═╡ 8b04f3c6-a403-466b-b556-542a266bf8dd
begin
	function time_derivative(values, time)
		n, m = size(values)
		deriv = zeros(n, m)
		
		for j in 1:m
			deriv[1, j] = (values[2, j] - values[1, j]) / (time[2] - time[1])
			
			for i in 2:n-1
				deriv[i, j] = (values[i+1, j] - values[i-1, j]) / (time[i+1] - time[i-1])
			end
			
			deriv[n, j] = (values[n, j] - values[n-1, j]) / (time[n] - time[n-1])
		end
		
		return deriv
	end
	
	ddq_sol = time_derivative(dq_sol, collect(tN))
end

# ╔═╡ a8bf26b6-1fb1-4e4c-8f9f-1150687e0628
begin
	θ2 = q_sol[:, 6]
	θ3 = q_sol[:, 9]
...
# ╔═╡ 248f8989-ff17-4d27-bf12-02882c03d7ce
begin
	plot(
		tN,
		α2,
		...
		title = "Angular Acceleration of the Crankshaft",
		label = "α₂ crankshaft",
		grid = true
	)
end

# ╔═╡ ca791aa9-b071-4779-aafe-29abf729cd65
begin
	plot(
		tN,
		α3,
		...
		label = "α₃ connecting rod",
		grid = true
	)
end

# ╔═╡ 95fb76f1-5a36-4684-9a58-016ce1e5f55f
begin
	plot(
		tN,
		α2,
		...
		xlabel = "Time (s)",
		ylabel = "Angular acceleration (rad/s²)",
		title = "Crankshaft vs Connecting Rod Angular Acceleration",
		label = "α₂ crankshaft",
		grid = true
	)
...
# ╔═╡ a2122175-74d8-42cb-a830-11cc5a04fb86
begin
	plot(
		tN,
		ω2,
		...
		title = "Angular Velocity of Rotating Parts",
		label = "ω₂ crankshaft",
		grid = true
	)
	...
	plot!(
		...
		ω3,
		linewidth = 3,
		label = "ω₃ connecting rod"
	)
end

# ╔═╡ a2117be8-3ad6-483c-923e-36a34c4b3e01
begin
	p1 = plot(
		tN,
		x_piston,
...
# ╔═╡ 06275ee7-876a-42c8-b316-3818300c3478
begin
	plot(
		tN,
		residual,
		...
		xlabel = "Time (s)",
		ylabel = "||C(q,t)||",
		title = "Constraint Residual Check",
		yscale = :log10,
		label = "constraint residual",
		grid = true
	)
end

# ╔═╡ f0dc862f-612e-4108-ac4f-255c328c5f1f
begin
	function joint_locations(q)
		x1, y1, θ1, x2, y2, θ2, x3, y3, θ3, x4, y4, θ4 = q
		
		ground = [0.0, 0.0]
		crank_tip = [L2*cos(θ2), L2*sin(θ2)]
		piston = [x4, y4]
		
		return ground, crank_tip, piston
	end
	
	function plot_piston_crank_snapshots(q_sol)
		snapshot_indices = round.(Int, LinRange(1, size(q_sol, 1), 8))
		
		P = plot(
			xlabel = "x-position (m)",
			ylabel = "y-position (m)",
			title = "Piston-Crank Motion Snapshots",
			aspect_ratio = :equal,
			grid = true,
			legend = :outerright
		)
		...
		for idx in snapshot_indices
			ground, crank_tip, piston = joint_locations(q_sol[idx, :])
			
			plot!(
				P,
				[ground[1], crank_tip[1]],
				[ground[2], crank_tip[2]],
				linewidth = 3,
				label = "crank t=$(round(tN[idx], digits=3))"
			)
			
			plot!(
				P,
				[crank_tip[1], piston[1]],
				[crank_tip[2], piston[2]],
				linewidth = 2,
				label = false
			)
...
# ╔═╡ 7c2c5609-5ac4-410c-a788-793b2c2b5a0e
begin
	plot_piston_crank_snapshots(q_sol)
end

# ╔═╡ 0f37d4c0-b87c-46a3-83a5-61cce072b5b1
md"""
# Textbook-Style Constraint Coefficient Check

The textbook equation you attached lists specific nonzero Jacobian coefficients, such as terms involving:
...
These terms come from differentiating the revolute-joint constraints with respect to the rotation coordinates.

For example, terms like

``\frac{L_2}{2}\cos\theta_2``

or

``\frac{L_2}{2}\sin\theta_2``

come from differentiating the crank endpoint location.

The same happens for the connecting rod with ``L_3/2``.

The cell below evaluates the constraint Jacobian at the first time step so the coefficient structure can be checked.
"""

# ╔═╡ 159da023-c911-4716-95d6-541a532782b6
begin
	Cq_initial = jacobian_q(C, q_sol[1, :], tN[1])
	
	# Rows and columns related to θ₂ and θ₃
	# Column 6 is θ₂
	# Column 9 is θ₃
	Cq_initial[:, [6, 9]]
end
...
manifest_format = "2.0"
project_hash = "910ad693f9117780b8834264e224abe358a1aace"
